...
<!-- ======= Header ======= -->
  <header id="header" class="header fixed-top d-flex align-items-center">
    ...
    <div class="search-bar">
      <form class="search-form d-flex align-items-center" method="POST" action="userList">
...
<main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <form action="newUser" method="get" class="text-md-end">
   		 <button type="submit" class="btn m-1 text-light btn-sm" style="background-color: #90e0ef;">New User</button>
	  </form>
    </div><!-- End Page Title -->
      ...
      <div class="col-12">
        <div class="card top-selling overflow-auto">
           <div class="card-body pb-0">
    			<table class="table table-info ms-2" style="width:99%">
        <thead>
            <tr>
                <th>Id</th>
			    <th>UserName</th>
        ...
        <tbody>
            <% for (User user : (List<User>) request.getAttribute("userList")) { %>
                <tr class="table-light">
                    <td class="table-light"><%= user.getId() %></td>
                    <td class="table-light"><%= user.getUsername() %></td>
                    <td class="table-light"><%= user.getEmail() %></td>
                    <td class="table-light"><%= user.getRole() %></td>
                    <td class="table-light">
                        <a href="editUser?id=<%= user.getId() %>" class="btn"><i class="bi bi-pencil-square"></i></a>
                        <a onclick=" return confirm('Are you sure?')" href="deleteUser?id=<%= user.getId() %>" class="btn"><i class="bi bi-trash-fill"></i></a>
                    </td>
                </tr>
            <% } %>
        </tbody>
